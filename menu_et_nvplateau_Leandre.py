...
class MainMenu:
    def __init__(self):
        self.TITRE = "BIENVENUE SUR PYTHOMINOES"
        self.message = "Appuyez sur 'Entrée' pour commencer"
        pyxel.load("tilemap.pyxres")
        self.pieces_cascade_liste = []  # Liste des pièces en cascade
        self.val = randint(1, 12) * 16 + 8  # Valeur initiale
        self.font_scale = 3  # Échelle pour agrandir le texte
    ...
    def draw(self):
        ...
        pyxel.cls(0)  # Efface l'écran avec une couleur de fond
        pyxel.text(140, (1 * 30 + 200) // 2, self.TITRE, pyxel.frame_count % 16)  # Affiche le message
        pyxel.text(125, (10 * 30 + 200) // 2, self.message, col= 2)  # Affiche le message d'instruction
        # Dessiner les pièces en cascade
        pyxel.text((width - len(self.message) * 4) // 2, height - 20, self.message, col=2)  # Affiche le message d'instruction en bas au centre
